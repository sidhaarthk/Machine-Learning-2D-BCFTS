...
# ============================================================
#  Neural net: u_eps(s), u_sig(s)
# ============================================================

class CharacterNet(nn.Module):
    """
    Small MLP that outputs (u_eps(s), u_sig(s)) and we exponentiate
    on top of IR envelopes to get χ_ε(t), χ_σ(t).
    """
...
def A_eps(t: torch.Tensor) -> torch.Tensor:
    ...
    """
    return torch.exp(-23.0 * pi * t / 24.0)   # Ising χ_ε IR asymptotics
...
def A_sig(t: torch.Tensor) -> torch.Tensor:
    ...
    """
    return torch.exp(-pi * t / 12.0)          # Ising χ_σ IR asymptotics
...
def theta3(q, N=N_THETA):
    """
    θ_3(0 | τ) = 1 + 2 Σ_{n=1}^∞ q^{n^2},  with q = e^{π i τ}.
    Here q is passed directly (already exponentiated).
    """
    q_shape = q.shape
...
def theta4(q, N=N_THETA):
    """
    θ_4(0 | τ) = 1 + 2 Σ_{n=1}^∞ (-1)^n q^{n^2},  q = e^{π i τ}.
    """
    q_shape = q.shape
    logq = torch.log(q).reshape(1, -1)
    n = torch.arange(1, N+1, device=q.device, dtype=q.dtype).reshape(-1, 1)
...
def theta2(q, N=N_THETA):
    """
    θ_2(0 | τ) = 2 Σ_{n=0}^∞ q^{(n+1/2)^2}, q = e^{π i τ}.
    """
    q_shape = q.shape
    logq = torch.log(q).reshape(1, -1)
...
def eta(q, N=N_ETA):
    """
    Dedekind η(q) in product form:
      η(q) = q^(1/24) ∏_{n=1}^N (1 - q^n), q = e^{2π i τ}.
    For q ≤ e^{-2π*0.1} ~ 0.534 and N=400, truncation error is tiny.
    """
    q_shape = q.shape
    logq = torch.log(q).reshape(1, -1)
    n = torch.arange(1, N+1, device=q.device, dtype=q.dtype).reshape(-1, 1)
...
# ============================================================
#  χ_1(t) in PyTorch (used in Cardy only)
# ============================================================

def chi_1(t: torch.Tensor) -> torch.Tensor:
    """
    Ising vacuum character:
      χ_1(t) = (sqrt θ3 + sqrt θ4) / (2 sqrt η),
    evaluated at τ = i t with:

      q_theta = e^{-π t},   for θ_2, θ_3, θ_4,
      q_eta   = e^{-2π t},  for η.

    This matches the mpmath reference in check_chi1_accuracy().
    """
...
# ============================================================
#  NN characters χ_ε, χ_σ from model
# ============================================================

def chi_eps_sig_from_model(s: torch.Tensor):
    """
    Given s = log t, produce χ_ε(t), χ_σ(t) from the model with IR envelopes.
    Returns (chi_eps, chi_sig, u_eps, u_sig).
    """
...
# ============================================================
#  Cardy residuals for Ising
# ============================================================

def cardy_residuals(s: torch.Tensor):
    """
    Cardy constraints for the Ising model, written in terms of
    characters at t and 1/t (with τ = i t, S: τ -> -1/τ ⇒ t -> 1/t).

      χ_1(1/t)   χ_ε(1/t)   χ_σ(1/t)
      χ_1(t)     χ_ε(t)     χ_σ(t)

    are related by the Ising S-matrix. We encode:

      χ_1(t)   = 1/2 χ_1(1/t) + 1/2 χ_ε(1/t) + 1/√2 χ_σ(1/t)
      χ_ε(t)   = 1/2 χ_1(1/t) + 1/2 χ_ε(1/t) - 1/√2 χ_σ(1/t)
      χ_σ(t)   = 1/√2 χ_1(1/t) - 1/√2 χ_ε(1/t)

    plus a redundant combination R6 which stabilizes training.
    """
    t = torch.exp(s).squeeze(-1)
    t_inv = torch.exp(-s).squeeze(-1)

    chi_eps_t, chi_sig_t, _, _ = chi_eps_sig_from_model(s)
    chi1_t = chi_1(t)

    chi_eps_inv, chi_sig_inv, _, _ = chi_eps_sig_from_model(-s)
...
# ============================================================
#  Anchor: fix overall normalization / branch choice
# ============================================================
...
def chi_eps_exact(t: torch.Tensor) -> torch.Tensor:
    ...
    with τ = i t, q_theta = e^{-π t}, q_eta = e^{-2π t}.
    """
    q_theta, q_eta = ising_nomes(t)
...
def anchor_loss(t0=3.0, weight=10.0):
    """
    Penalise deviation of NN χ_ε, χ_σ from exact θ/η characters
    at a single t-value (or a small set if you want to extend it).

    This kills the overall normalization degeneracy (and sign
    ambiguities from the square roots).
    """
    t0_torch = torch.tensor([t0], dtype=torch.float64, device=device)
    s0 = torch.log(t0_torch).unsqueeze(-1)   # shape (1,1)

    # NN estimates
    chi_eps_nn, chi_sig_nn, _, _ = chi_eps_sig_from_model(s0)

    # Exact θ/η values
    chi_eps_ex = chi_eps_exact(t0_torch)
    chi_sig_ex = chi_sig_exact(t0_torch)

    mse = ((chi_eps_nn - chi_eps_ex)**2 + (chi_sig_nn - chi_sig_ex)**2).mean()
    return weight * mse
...
def asymptotic_regularisation(batch_size=256, band=0.4):
    """
    Encourage u_eps,u_sig -> 0 near the IR end t ~ T_MAX, so that
    χ_ε,χ_σ follow the chosen envelopes there.
    """
    s = sample_s(batch_size)
    mask = s[:, 0] > (S_MAX - band)
    if not mask.any():
    ...
    s_sel = s[mask]
    _, _, u_eps, u_sig = chi_eps_sig_from_model(s_sel)
...
def smoothness_regularisation(batch_size=256, delta=0.05):
    ...
    _, _, u_eps, u_sig = chi_eps_sig_from_model(s)
    _, _, u_eps_p, u_sig_p = chi_eps_sig_from_model(s_plus)
...
def check_chi1_accuracy():
    ...
    print(f"Max relative error χ₁ (PyTorch vs 50-dps mpmath) on [{T_MIN}, {T_MAX}]: {max_rel:.3e}")
...
def evaluate_and_plot():
    ...
    with torch.no_grad():
        ...
        chi_eps_nn, chi_sig_nn, _, _ = chi_eps_sig_from_model(s_grid)
    ...
    axes[0].loglog(t_np, eps_ex_np, label=r"$\chi_\varepsilon exact", linewidth=2)
    axes[0].loglog(t_np, eps_nn_np, "--", label="NN", linewidth=2)
    axes[0].set_xlabel("t")
    axes[0].set_ylabel(r"$\chi_\varepsilon$")
    axes[0].grid(True, which="both", ls="--", alpha=0.4)
    axes[0].legend()

    axes[1].set_title(r"$\chi_\sigma(t)$")
    axes[1].loglog(t_np, sig_ex_np, label=r"$\chi_\sigma exact", linewidth=2)
    axes[1].loglog(t_np, sig_nn_np, "--", label="NN", linewidth=2)
    axes[1].set_xlabel("t")
    axes[1].set_ylabel(r"$\chi_\sigma$")
...
if __name__ == "__main__":
    print("Training NN on Cardy conditions with correct θ/η conventions…")
    train(num_steps=10000, batch_size=256, lr=1e-3)

    print("Checking χ₁(t) precision against 50-dps mpmath reference…")
    check_chi1_accuracy()

    print("Plotting NN vs θ/η characters (ε, σ)…")
    evaluate_and_plot()